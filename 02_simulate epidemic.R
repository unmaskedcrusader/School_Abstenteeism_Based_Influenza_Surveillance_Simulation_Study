...
sim12epi <- function(b, sus, spar, numinf.catch){
  ...
  for(i in 1:12){
    ...
    samp <- stratified(individuals, "catchID", size = numinf.catch)$individualID
    ...
    first.inf[individuals$individualID %in% samp] <- start + floor(runif(numinf.catch*num.catch, 0, 15))
...
epi_series1 <- sim12epi(b=3, sus=.0019, spar=0, numinf.catch = 2)
