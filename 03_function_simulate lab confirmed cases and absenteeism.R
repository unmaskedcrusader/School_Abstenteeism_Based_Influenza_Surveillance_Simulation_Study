...
#### Create lagged datasets for catchment area and region wide modelling ####
# Input: a list consisting of a series/replication of simulated epidemics, and indiviudal information
# Output: A catchment area dataset and a region wide dataset ready for modeling
#   Output dataframe columns: 
#     Date - day in the school year (1-300)
#     ScYr - School year (1 - 10)
#     catchID - Catchment id factor (only present in the catchment dataset)
#     Actual.case - number of cases, including those that are not laboratory confirmed cases.  
#       This is not required for modelling, but can provide useful information.
#     case.elem - number of cases that are elementary school students,
#       including those that are not laboratory confirmed cases.
#       This is not required for modelling, but can provide useful information.
#     pct.absent - daily average percent of absent students
#     absent - count of total absent students
#     absent.sick - count of students absent due to illness
#       This is not required for modelling, but can provide useful information.
#     Case.No - Number of laboratory confirmed cases.  Used to create reference date
#     Case - Boolean for if there is a case or not that day.  
#       Used as the reponse variable in the logistic regression models.
#     sinterm - seasonal term in logistic regression models.
#     costerm - seasonal term in logistic regression models.
#     window - indicator for days within the "True Alarm" window (used for FAR and ADD calculations)
#     ref.date - indicator for reference date.
#     lag0 - lag15 - lagged daily average percent of absent students

model.data <- function(epi.series, individual.data){
...
#### Plot actual cases, reported cases, and absenteeism ####
plot.epi.info <- function(master.data, ScYr=1, catchment=NULL){
  
  #subset data for school year (and catchment if applicable)
  master.data <- master.data[master.data$ScYr == ScYr,]
  if(!is.null(catchment)){
    master.data <- master.data[master.data$catchID %in% catchment,]
  }
  
  #plot cases (lab confirmed and total cases)
  par(mar = c(5, 4, 4, 4) + 0.3)
  plot(master.data$Date, master.data$Actual.case
       , type="l", col="grey32"
       , xlab = "Time (days)", ylab = "Cases")
  points(master.data$Date, master.data$Actual.case
         , pch=16, cex=.66, col="grey32")
  lines(master.data$Date, master.data$Case.No
        , col="red")
  points(master.data$Date, master.data$Case.No
         , pch=16, cex=.66, col="red")

  # second axis for absenteeism percentage
  par(new = TRUE)
  plot(master.data$Date, master.data$pct.absent*100
       , type = "l", col="blue", axes = FALSE
       , bty = "n", xlab = "", ylab = "")
  points(master.data$Date, master.data$pct.absent*100
         , pch=16, cex=.66, col="blue")
  axis(side=4, col="blue", col.axis="blue", at = pretty(range(c(0,master.data$pct.absent*100))))
  mtext("Percent Absent", side=4, line=3, col="blue")
  
  legend("topright", legend = c("Actual Cases", "Reported Cases", "Average absenteeism"), bty="n", pch=c(16,16,16), lty=c(1,1,1), pt.cex=c(.66,.66,.66), col=c("grey32", "red", "blue"))
}
